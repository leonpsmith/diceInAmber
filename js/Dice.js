...
var imageSrc=nil;
var diceNum=nil;
((($receiver = smalltalk.send(self['@element'], "_isNil", [])).klass === smalltalk.Boolean) ? ($receiver ? (function(){return (self['@element']=smalltalk.send(smalltalk.send(html, "_div", []), "_id_", [unescape("body%20%3E%20dice")]));})() : nil) : smalltalk.send($receiver, "_ifTrue_", [(function(){return (self['@element']=smalltalk.send(smalltalk.send(html, "_div", []), "_id_", [unescape("body%20%3E%20dice")]));})]));
smalltalk.send(self['@element'], "_empty", []);
smalltalk.send(self['@element'], "_append_", [(function(){return smalltalk.send((1), "_to_do_", [smalltalk.send(self['@model'], "_size", []), (function(index){(diceNum=smalltalk.send(self['@model'], "_at_", [index]));(imageSrc=smalltalk.send(self['@imagePaths'], "_at_", [diceNum]));return (function($rec){smalltalk.send($rec, "_height_", [(100)]);smalltalk.send($rec, "_width_", [(100)]);return smalltalk.send($rec, "_style_", [unescape("margin-right%3A15px%3B")]);})(smalltalk.send(html, "_img_", [imageSrc]));})]);})]);
return self;},
args: ["html"],
source: unescape('renderOn%3A%20html%0A%09%22we%20should%20already%20know%20the%20dice%20that%20we%20are%20displaying%20by%20asking%20the%20model%20%28we%20start%20with%20a%20set%20number%2C%20but%20the%20player%20can%20lose%20one%20at%20each%20turn%29.%20We%20also%20save%20the%20HTML%20element%20%28usually%20a%20div%29%20so%20we%20can%20replace%20its%20contents.%0A%09diceLeft%20is%20a%20Dice%20Object%20that%20contains%20an%20array%20of%20values%20representing%20the%20value%20of%20each%20die%20and%20allows%20us%20to%20choose%20the%20correct%20image%20for%20that%20die%20from%20the%20collection%20of%20imagePaths%22%0A%0A%09%7C%20imageSrc%20diceNum%20%20%7C%0A%0A%09%09element%20isNil%20%20ifTrue%3A%5B%0A%09%09%09element%20%3A%3D%20html%20div%20id%3A%20%27body%20%3E%20dice%27.%0A%09%09%09%22element%20id%3A%20%27dice%27%20.%22%0A%09%09%09%22html%20appendToJQuery%3A%20%27%23dice%27%20asJQuery.%22%0A%09%09%5D.%0A%09element%20empty.%0A%09element%20append%3A%5B%0A%09%091%20to%3A%20%28model%20size%29%20do%3A%5B%20%3Aindex%20%7C%0A%09%09%09diceNum%20%3A%3D%20model%20at%3A%20index.%0A%09%09%09imageSrc%20%3A%3D%20imagePaths%20at%3A%20diceNum.%0A%09%09%09%28html%20img%3A%20imageSrc%29%20%20height%3A100%3B%20width%3A%20100%3B%20style%3A%20%27margin-right%3A15px%3B%27.%0A%09%09%5D.%0A%09%5D.%0A'),
messageSends: ["ifTrue:", "isNil", "id:", "div", "empty", "append:", "to:do:", "size", "at:", "height:", "width:", "style:", "img:"],
referencedClasses: []
}),
smalltalk.DiceView);
...
return self;},
args: [],
source: unescape('initialize%0A%09%22comment%20stating%20purpose%20of%20message%22%0A%0A%20%20%20%20%20%20%20%20super%20initialize.%0A%09self%20createImagePaths.%0A%09'),
messageSends: ["initialize", "createImagePaths"],
referencedClasses: []
}),
smalltalk.DiceView);
